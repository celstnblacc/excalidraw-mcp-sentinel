...
443 tests across unit, API, WebSocket, and regression suites. Run `npm test` or `pnpm test`. CI runs `type-check` then `build` then `test` across Node 18/20/22.
...
- 443/443 tests passing; 4 regression tests cover previously crash-able sync paths
